...
SSHBridgeは、オープンソースで永久無料のセルフホスト型オールインワンサーバー管理プラットフォームです。単一の直感的なインターフェースを通じて、サーバーとインフラストラクチャを管理するマルチプラットフォームソリューションを提供します。SSHBridgeは、SSHターミナルアクセス、リモートデスクトップ制御（RDP、VNC、Telnet）、SSHトンネリング機能、リモートSSHファイル管理、およびその他多くのツールを提供します。SSHBridgeは、すべてのプラットフォームで利用可能なTermiusの完全無料でセルフホスト可能な代替ソリューションです。
...
- **プラットフォーム対応** - Webアプリ、デスクトップアプリケーション（Windows、Linux、macOS。SSHBridgeバックエンドなしでスタンドアロン動作可能）、PWA、iOS・Android専用モバイル/タブレットアプリとして利用可能です。
...
すべての予定機能については[Projects](https://github.com/nghoang1288/SSHBridge-Web)をご覧ください。コントリビュートをご希望の方は[Contributing](https://github.com/nghoang1288/SSHBridge-Web/blob/main/CONTRIBUTING.md)をご覧ください。
...
すべてのプラットフォームへのSSHBridgeのインストール方法については、SSHBridgeの[ドキュメント](https://docs.termix.site/install)をご覧ください。または、以下のサンプルDocker Composeファイルをご覧ください（リモートデスクトップ機能を使用する予定がない場合は、guacdとネットワークの設定を省略できます）：
...
SSHBridgeに関するヘルプや機能リクエストが必要な場合は、[Issues](https://github.com/nghoang1288/SSHBridge-Web/issues)ページにアクセスし、ログインして`New Issue`を押してください。Issueはできるだけ詳細に記述し、英語での記述が望ましいです。また、[Discord](https://discord.gg/jVQGdvHDrf)サーバーに参加してサポートチャンネルを利用することもできますが、応答時間が長くなる場合があります。
